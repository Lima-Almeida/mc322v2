...
public class ClientePF extends Cliente {
	...
	public double calculaScore() {
		...
		age = calcularIdade();
		if (age <= 30) {
			valor = calcseguro[0].fator() * calcseguro[1].fator() * listaVeiculos.size();
		}
		else if ((age > 30) && (age <= 60)) {
			valor = calcseguro[0].fator() * calcseguro[2].fator() * listaVeiculos.size();
		}
		else if (age > 60) {
			valor = calcseguro[0].fator() * calcseguro[3].fator() * listaVeiculos.size();
		}
		return valor;
	}
